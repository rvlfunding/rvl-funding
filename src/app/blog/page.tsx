...
const articles = [
  {
    date: 'May 2026',
    title: 'EB-1A Visa: Key Strategic Insights for 2026',
    excerpt:
  ...
  {
    date: 'January 2026',
    title: 'Asset Protection Without Theatrics',
    excerpt:
      'Trust and offshore layering where statute allows — without strategies that fail under creditor or regulatory review.',
    category: 'Wealth Management',
    readTime: '8 min',
  },
];
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
          {current + 1}–{Math.min(current + VISIBLE, total)} of {total} articles
        </p>
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button
            ...
            style={{
              ...
              background: current === 0 ? 'var(--mist-2)' : 'var(--white)',
              color: current === 0 ? 'var(--text-muted)' : 'var(--navy)',
              cursor: current === 0 ? 'not-allowed' : 'pointer',
              fontSize: '16px',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              transition: 'all 0.2s',
              boxShadow: current === 0 ? 'none' : '0 2px 8px rgba(10,37,64,0.08)',
            }}
          >
            ←
          </button>
          <button
            ...
            style={{
              ...
              borderRadius: '50%',
              border: '1px solid var(--border)',
              background: current >= maxIndex ? 'var(--mist-2)' : 'var(--navy)',
              color: current >= maxIndex ? 'var(--text-muted)' : 'var(--gold)',
              cursor: current >= maxIndex ? 'not-allowed' : 'pointer',
              fontSize: '16px',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              transition: 'all 0.2s',
              boxShadow: current >= maxIndex ? 'none' : '0 2px 8px rgba(10,37,64,0.15)',
            }}
          >
            →
          </button>
      ...
      <div style={{ overflow: 'hidden' }} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        <div style={{
          display: 'flex',
          gap: '1.5rem',
          transform: `translateX(calc(-${current} * (100% / ${VISIBLE} + 0.5rem)))`,
          ...
          {articles.map((article, idx) => (
            <article
              ...
              style={{
                minWidth: `calc((100% - ${(VISIBLE - 1) * 1.5}rem) / ${VISIBLE})`,
                flexShrink: 0,
                cursor: 'pointer',
                opacity: idx >= current && idx < current + VISIBLE ? 1 : 0.4,
      ...
      <section className="page-hero">
        <div className="container">
          ...
          <p className="hero-text">
            Longer essays for principals and family offices already running institutional processes —
            not generic marketing posts.
          </p>
