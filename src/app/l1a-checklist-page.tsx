    ...
    <main className="page-shell">
      ...
      <section style={{ padding: 'clamp(3rem, 7vw, 5rem) 0', background: 'var(--mist-2)' }}>
        <div className="container">
          ...
          {/* OVERVIEW */}
          <div style={cardStyle}>
            <h2 style={cardTitle}>Overview — What is the L1-A Visa?</h2>
            <p style={{ fontSize: '0.95rem', color: 'var(--text-muted)', lineHeight: 1.8 }}>
              ...
              a common pathway to the EB-1C green card. This checklist covers key documents required for
              an L-1A petition (Form I-129).
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginTop: '1.25rem' }}>
              {[
                ...
                <div key={i} style={{ background: 'var(--mist-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '1rem' }}>
                  <p style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--gold)', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: '0.3rem' }}>{title}</p>
          ...
          {/* PART 1 — PETITIONER */}
          <div style={cardStyle}>
            <h2 style={cardTitle}>Part 1 — Petitioner Documents (U.S. Company)</h2>
            <table style={tableStyle}>
              ...
              <tbody>
                {[
                  ...
                  ['Premium processing (optional)', 'Form I-907 + $2,805 for 15-business-day processing'],
                  ...
                  ['U.S. company financial statements', 'Balance sheet and P&L for the most recent year'],
                  ['U.S. company organizational chart', "Showing beneficiary's proposed position and reporting structure"],
                  ...
                  ['Support letter from U.S. company', "Describing qualifying relationship and beneficiary's role"],
                ].map(([doc, notes], i) => (
          ...
          <div style={cardStyle}>
            <h2 style={cardTitle}>Part 2 — Qualifying Relationship Between U.S. &amp; Foreign Entities</h2>
            <table style={tableStyle}>
              <thead><tr><th style={th}>Document</th><th style={th}>Notes</th></tr></thead>
              <tbody>
                {[
                  ...
                  ['Affiliate / subsidiary agreement', 'If qualifying relationship is based on affiliation rather than direct ownership'],
                ].map(([doc, notes], i) => (
                  <tr key={i} style={{ background: i%2===0 ? 'var(--mist-2)' : 'var(--white)' }}>
                    <td style={{ ...td, fontWeight: 600, color: 'var(--navy)', width: '42%' }}>{doc}</td>
                    <td style={td}>{notes}</td>
          ...
          {/* PART 3 — BENEFICIARY EMPLOYMENT */}
          <div style={cardStyle}>
            <h2 style={cardTitle}>Part 3 — Beneficiary Employment History at Foreign Entity</h2>
            <table style={tableStyle}>
              <thead><tr><th style={th}>Document</th><th style={th}>Notes</th></tr></thead>
              <tbody>
                {[
                  ...
                  ['Board resolution or transfer authorization', 'Authorizing the transfer to the U.S. entity'],
                ].map(([doc, notes], i) => (
                  <tr key={i} style={{ background: i%2===0 ? 'var(--mist-2)' : 'var(--white)' }}>
                    <td style={{ ...td, fontWeight: 600, color: 'var(--navy)', width: '42%' }}>{doc}</td>
                    <td style={td}>{notes}</td>
          ...
          <div style={cardStyle}>
            <h2 style={cardTitle}>Part 4 — Beneficiary Personal Documents</h2>
            <table style={tableStyle}>
              <thead><tr><th style={th}>Document</th><th style={th}>Notes</th></tr></thead>
              <tbody>
                {[
                  ['Valid passport (bio page copy)', 'Must be valid for at least 6 months beyond intended stay'],
                  ['Copies of all prior U.S. visas', 'Including expired visas'],
                  ['I-94 arrival/departure record', 'Download from cbp.dhs.gov/I94'],
                  ['Prior I-797 approval notices', 'Any prior L, H, O, or other nonimmigrant status'],
                  ['Diploma / degree certificates', 'Educational background supporting executive or managerial role'],
                  ['CV / résumé', 'Full employment history covering both foreign and U.S. entities'],
          ...
          {/* PART 5 — NEW OFFICE */}
          <div style={cardStyle}>
            <h2 style={cardTitle}>Part 5 — New Office L-1A (Additional Requirements)</h2>
          ...
          {/* PART 6 — EB-1C PATHWAY */}
          <div style={cardStyle}>
            <h2 style={cardTitle}>Part 6 — L-1A to EB-1C Green Card Pathway</h2>
            <p style={{ fontSize: '0.88rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>
              L-1A holders in executive or managerial roles may be sponsored for the EB-1C immigrant visa by their U.S. employer. No PERM labor certification required.
            </p>
            <table style={tableStyle}>
              <thead><tr><th style={th}>Step</th><th style={th}>Details</th></tr></thead>
              <tbody>
                {[
                  ['Step 1 — I-140 Petition', 'U.S. employer files Form I-140 (Immigrant Petition for Alien Workers). No PERM required for EB-1C.'],
                  ['Step 2 — Priority Date', 'EB-1C is generally current for most countries. Check the monthly Visa Bulletin for India and China backlogs.'],
                  ['Step 3 — Adjustment of Status', 'File Form I-485 concurrently with I-140 if a visa number is immediately available.'],
                  ['Step 4 — Consular Processing', 'Alternative to I-485 if the beneficiary is outside the U.S. Apply at the nearest U.S. consulate.'],
                  ['Key Advantage', 'No PERM required. Typically faster than EB-2 or EB-3 for most nationalities.'],
                ].map(([step, details], i) => (
                  <tr key={i} style={{ background: i%2===0 ? 'var(--mist-2)' : 'var(--white)' }}>
          ...
          <div style={{ marginTop: '2.5rem', padding: '1.5rem 2rem', background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 'var(--radius)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem', flexWrap: 'wrap' as const }}>
              <div style={{
                ...
                padding: '0.4rem 0.9rem', borderRadius: '6px', letterSpacing: '0.1em',
              }}>
                RVL FUNDING
              </div>
            ...
            <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', lineHeight: 1.7 }}>
              ...
              USCIS policy updates, and consular requirements. Always consult a qualified U.S. immigration attorney before filing.
              © 2026 RVL Funding · All rights reserved.
            </p>
          </div>
...
const cardStyle: React.CSSProperties = {
  ...
  borderRadius: 'var(--radius)', padding: 'clamp(1.25rem, 3vw, 2rem)',
  boxShadow: '0 8px 32px rgba(10,37,64,0.05)', marginBottom: '1.5rem',
};
...
const th: React.CSSProperties = {
  background: 'var(--navy)', color: 'var(--gold)', padding: '0.6rem 0.9rem',
  textAlign: 'left', fontSize: '0.72rem', fontWeight: 700,
  letterSpacing: '0.08em', textTransform: 'uppercase' as const,
};
const td: React.CSSProperties = {
  padding: '0.6rem 0.9rem', color: 'var(--text-muted)',
  borderBottom: '1px solid var(--border)', verticalAlign: 'top', lineHeight: 1.7,
};
